...
/* harmony default export */ __webpack_exports__["default"] = (MatchHeight$1);
...
/* harmony import */ var matchheight__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! matchheight */ "./node_modules/matchheight/dist/MatchHeight.module.js");
/* harmony import */ var lodash_es_max__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! lodash-es/max */ "./node_modules/lodash-es/max.js");
...
var removePrefix = function removePrefix(string) {
  return string.startsWith('.product_') ? string.slice(13).replace(/-/g, ' ') : string;
};

var formatTag = function formatTag(string) {
  var value = '';
...
var addTag = function addTag(elementId, html) {
  var id = elementId.replace('.', '');
  var tag = document.createElement('a');
  var closeButton = document.createElement('span');
  var container = document.querySelector('.wc-isotope-active-filters');
  ...
  tag.addEventListener('click', function (event) {
    event.preventDefault();
    inclusives = inclusives.filter(function (value) {
...
var removeTag = function removeTag(id) {
  id = id.replace('.', '');
  var el = document.querySelector(".wc-isotope-active-filters #".concat(id));
  var matchingFilterEl = document.querySelector(".filter-link-group #".concat(id));
...
var initSearch = function initSearch() {
  var clearButton = document.querySelector('.wc-isotope-search .clear');
  ...
  quicksearch.addEventListener('keyup', Object(lodash_es_debounce__WEBPACK_IMPORTED_MODULE_3__["default"])(function () {
...
var initSort = function initSort() {
  var select = document.querySelector('#wc-isotope-sort');
  ...
  matchheight__WEBPACK_IMPORTED_MODULE_1__["default"].update();
};

var runFilter = function runFilter() {
  isotope.arrange({
    filter: function filter(el) {
      var rawFilterValue = filters['links'];
      var filterValues = rawFilterValue ? rawFilterValue.replace('.', '') : false;
      var searchValue = filters['search'] && filters['search'] !== '' ? filters['search'] : false;

      if (searchValue) {
        var titleEl = el.querySelector('.woocommerce-loop-product__title');
        var title = titleEl ? titleEl.innerText.toLowerCase() : false;
      ...
      if (filterValues) {
        var isMatch = true;
        inclusives.forEach(function (value) {
          var cssClass = value.replace('.', '');
...
var updateProductTitleHeights = function updateProductTitleHeights() {
  var heights = [];
  var productTitleEls = document.querySelectorAll('.woocommerce-loop-product__title');
  ...
  var maxHeight = Object(lodash_es_max__WEBPACK_IMPORTED_MODULE_2__["default"])(heights);
  document.documentElement.style.setProperty('--product-title-min-height', "".concat(maxHeight, "px"));
};
...
var toggleIsHiddenClassForFilters = function toggleIsHiddenClassForFilters() {
  ...
  toggle.addEventListener('click', function (event) {
    var target = document.querySelector('.wc-isotope-filters');
    event.preventDefault();
...
var updateEachCounts = function updateEachCounts() {
  var filterLinks = document.querySelectorAll('.filter-link');
  ...
  filterLinks.forEach(function (link) {
    ...
    var formattedFilterValue = filterValue.replace('.', '');
    var total = items.filter(function (_ref) {
      var element = _ref.element;
      return element.classList.contains(formattedFilterValue);
    }).length;
...
var updateTotalCount = function updateTotalCount() {
  ...
  isotope.on('arrangeComplete', function (filteredItems) {
    ...
    el.innerText = "Showing all ".concat(total, " results");
    updateEachCounts();
  });
};
...
var setup = function setup() {
  ...
  var filterLinkGroups = document.querySelectorAll('.filter-link-group');

  if (!filterLinkGroups || !filterLinkGroups.length || !el) {
    return;
  }

  isotope = new isotope_layout__WEBPACK_IMPORTED_MODULE_0___default.a(el, {
    ...
    getSortData: {
      ...
      price: function price(el) {
        var priceEl = el.querySelector('.woocommerce-Price-amount');
        var salePriceEl = el.querySelector('ins .woocommerce-Price-amount');

        if (!priceEl && !salePriceEl) {
          return 0;
        }

        var price = parseInt(priceEl.innerText.replace(',', '.'), 10);
        var salePrice = salePriceEl ? parseInt(salePriceEl.innerText.replace(',', '.'), 10) : false;
        var amount = salePrice ? salePrice : price;
        return amount;
      },
      priceDesc: function priceDesc(el) {
        var priceEl = el.querySelector('.woocommerce-Price-amount');
        var salePriceEl = el.querySelector('ins .woocommerce-Price-amount');
  ...
  filterLinkGroups.forEach(function (group) {
...
var initIsotope = function initIsotope() {
  ...
  matchheight__WEBPACK_IMPORTED_MODULE_1__["default"].init();
  setup();
  initSearch();
  initSort();
